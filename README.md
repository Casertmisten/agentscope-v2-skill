...
- **RAG 知识库**（v2.0.3+）：`agentscope.rag` 模块（`KnowledgeBase` 句柄 + `QdrantStore` 向量库 + Parser/Chunker 索引管线），`RAGMiddleware` 提供 static（自动注入）与 agentic（工具驱动）两种检索模式
...
- **Middleware**：拦截 reply/reasoning/acting/model_call，含内置 `TTSMiddleware` / `ReplyBudgetControlMiddleware` / `Mem0Middleware`（mem0 跨会话长期记忆，v2.0.3+）
