...
- **Workspace**：`LocalWorkspace` / `DockerWorkspace` / `E2BWorkspace` / `K8sWorkspace` / `OpenSandboxWorkspace`（后两者 v2.0.4+），均支持内置工具（Bash/Read/Write/Edit/Grep/Glob）在容器/云沙箱/K8s Pod 执行，Backend 抽象（`LocalBackend` / `DockerBackend` / `E2BBackend` / `K8sBackend` / `OpenSandboxBackend`）
