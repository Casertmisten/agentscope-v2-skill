...
- **Pipeline 流水线**（v2.0.8）：`agentscope.pipeline` 的 `GoalPipeline` 把 executor（执行者）与 verifier（校验者）编成目标达成循环——执行报告 → 结构化验收 pass/fail/impossible → fail 带反馈重试至 `max_iters`；对外暴露与 `Agent` 相同的 `reply_stream` 事件流，支持 HITL 暂停恢复
