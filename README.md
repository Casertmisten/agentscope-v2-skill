...
- **Model 配置**：ChatModelBase、重试、fallback、Omni 模型音频输出、`extra_body` 透传（v2.0.3+）、Anthropic 推理控制（`thinking_mode` / `thinking_display` / `reasoning_effort`，v2.0.6+）、Volcengine Ark 豆包模型（`VolcengineChatModel`，v2.0.8+）
...
- **Toolkit / ToolBase**：工具注册、内置工具（Bash, Read, Write 等；Windows 下 `PowerShell`，v2.0.5+）、`AskUser` 多选提问外部工具（向用户收集偏好/澄清意图/做决策，结构化答案经 `ToolResultBlock.metadata` 回传，`ToolBase.metadata_schema` + `check_external_result` 外部结果 schema 校验，v2.0.8+）、`FunctionTool` 自定义 `input_schema`（JSON schema / pydantic BaseModel，v2.0.7+）与 `permission` 构造参数（直接指定权限决策，默认 ASK，v2.0.8）、`ToolMiddlewareBase` 工具级洋葱中间件（v2.0.3+）、工具结果多模态传递（支持的媒体原样传给模型——OpenAI Responses API 起原生放入 `function_call_output`，v2.0.7+，其余 API 提升为 user 消息；不支持的媒体回退为 URL 引用/临时文件路径）、工具参数 schema 引导修复（按 `input_schema` 自动修复 LLM 给错的参数类型，v2.0.8）
