...
- **终端 UI TUI**（v2.0.8+）：`agentscope.tui` 基于 Textual 的全屏富文本聊天界面——`launch_tui` 一行启动（Agent / pipeline 均可传入，流式渲染 + 工具确认/外部执行/`AskUser` 交互控件 + 中断，`/exit` 退出），`ChatUI` / `MessagesUI` 组件可嵌入自己的 Textual 应用；需安装 `agentscope[tui]` extra
