...
- **Pipeline** (v2.0.8): `GoalPipeline` from `agentscope.pipeline` composes an executor and a verifier into a goal-achievement loop — execution report → structured verification pass/fail/impossible → fail retries with feedback up to `max_iters`; it exposes the same `reply_stream` event stream as `Agent` and supports HITL pause/resume
